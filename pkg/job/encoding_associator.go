...
// encodingAssociator contains for set of dimensions, the matched values and resources. Each set of dimensions
// is expressed as a concatenation of their names, order lexicographically, and using a separator in-between.
type encodingAssociator struct {
...
// newEncodingMetricsToResourceAssociator creates a new metricsToResourceAssociator given a set of dimensions regexs that can extract
// dimensions from a resource ARN, and a set of resources from which to extract.
func newEncodingMetricsToResourceAssociator(dimensionRegexps []*regexp.Regexp, resources []*model.TaggedResource) *encodingAssociator {
...
func (a *encodingAssociator) associateMetricsToResources(cwMetric *cloudwatch.Metric) (*model.TaggedResource, bool) {
	matches := []match{}
