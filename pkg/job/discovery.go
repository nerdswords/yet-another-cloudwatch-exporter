...
func runDiscoveryJob(
	...
	for i := 0; i < metricDataLength; i += maxMetricCount {
		...
		g.Go(func() error {
			...
			data := clientCloudwatch.GetMetricData(gCtx, logger, input, svc.Namespace, length, job.Delay, job.RoundingPeriod, addHistoricalMetrics)
			if data != nil {
				mu.Lock()
	...
	getMetricDatas = mapResultsToMetricDatas(getMetricDataOutput, getMetricDatas, logger)
...
func mapResultsToMetricDatas(output [][]cloudwatch.MetricDataResult, datas []*model.CloudwatchData, logger logging.Logger) []*model.CloudwatchData {
	...
	for _, data := range output {
		...
		for _, metricDataResult := range data {
			...
			if metricData.MetricID == nil {
				if *metricData.AddHistoricalMetrics {
					// Use the previousIdx to make a copy
					if previousID == metricDataResult.ID {
