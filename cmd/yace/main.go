...
var (
	addr                  = flag.String("listen-address", exporter.GetEnv("addr", ":5000").(string), "The address to listen on.")
	configFile            = flag.String("config.file", "config.yml", "Path to configuration file.")
	debug                 = flag.Bool("debug", exporter.GetEnv("debug", false).(bool), "Add verbose logging.")
	...
	scrapingInterval      = flag.Int("scraping-interval", exporter.GetEnv("scrapingInterval", 300).(int), "Seconds to wait between scraping the AWS metrics")
	metricsPerQuery       = flag.Int("metrics-per-query", exporter.GetEnv("metricsPerQuery", 500).(int), "Number of metrics made in a single GetMetricsData request")
